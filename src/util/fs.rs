...
use crate::{component::block::log, state::{Scanning, acquire, notify_redraw}};
...
pub async fn scan(mode: Scanning) -> Result<(), std::io::Error> {
	log::info("Scan spawned");
...
async fn add_duration(tab: String) {
	let app = acquire();
...
async fn scan_tab(index: usize) -> Result<(), std::io::Error> {
	let app = acquire();
	...
	if path.is_dir() {
		...
		add_duration(tab).await;
	}
	Ok(())
}

async fn scan_tabs() -> Result<(), std::io::Error> {
